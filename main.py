...
import functools
import re
from typing import List, Tuple, Dict, Optional, Callable, Set, Union
...
from utils.exceptions import VermissianError, NoCharacterError, NoGameError
...
def command_logging_decorator(command: Callable):
    ...
    @functools.wraps(command)
    async def wrapper(ctx: discord.ApplicationContext, * args, ** kwargs):
        log_message = f'Command {command.__name__} called in Guild {ctx.guild_id} ("{ctx.guild.name}") by {ctx.user.name} with args {args} and kwargs {kwargs}'[:3000]
...
def guild_required_decorator(command: Callable):
    ...
    @functools.wraps(command)
    async def wrapper(ctx: discord.ApplicationContext, * args, ** kwargs):
        if not ctx.guild_id in vermissian.player_guilds:
            raise NoGameError()
...
def character_required_decorator(command: Callable):
    ...
    async def wrapper(ctx: discord.ApplicationContext, * args, ** kwargs):
        if not ctx.user.name in vermissian.player_guilds[ctx.guild_id].character_sheets:
...
ichor-drowned is a product by Sillion L and Brendan McLeod, with whom I'm not affiliated, but they've generously allowed me to include their content here. You can buy it here: https://sillionl.itch.io/ichor-drowned''', ephemeral=True)
...
async def legal_command(
    ...
    await ctx.respond('''The Vermissian dice bot is an independent production by jaffa6 (me) and is not affiliated with Rowan, Rook and Decard. It is published under the RR&D Community License.
...
async def help_command(
    ...
    getting_started_message = f'{underline(bold("Getting Started"))}\n'
...
You can also run {code("/add_character")}, passing in the URL for your specific tab on the character tracker, to link your own character to yourself. You can only have one character per user at once.'''
    ...
    raw_pages.append(getting_started_message)

    commands_message = f'{underline(bold("Available Commands"))}\n'
    command_tokens = []
    ...
    raw_pages.append(commands_message)

    debugging_message = f'{underline(bold("Fixing Errors"))}\n'
    ...
    raw_pages.append(debugging_message)

    headers = [
        raw_page.split('\n')[0] for raw_page in raw_pages
    ]
...
async def list_characters_command(
    ...
    guild = vermissian.player_guilds[ctx.guild_id]

    if len(guild.character_sheets) == 0:
        message = 'No characters linked. You can use /add_character to do this.'
    else:
        tokens = []

        for discord_username, character in guild.character_sheets.items():
...
async def tag_command(
    ...
    lowercase_spire_tags = {name.lower(): value for name, value in SpireGame.TAGS.items()}

    lowercase_heart_tags = {name.lower(): value for name, value in HeartGame.TAGS.items()}

    lowercase_tag = tag.lower()

    matches = {}

    if lowercase_tag in lowercase_spire_tags:
        matches[System.SPIRE.value] = lowercase_spire_tags[lowercase_tag]

    if lowercase_tag in lowercase_heart_tags:
        matches[System.HEART.value] = lowercase_heart_tags[lowercase_tag]

    if ctx.guild_id in vermissian.player_guilds:
        guild = vermissian.player_guilds[ctx.guild_id]

        if guild.system.value in matches:
            message = f'{bold(tag)}: {matches[guild.system.value]}'
        else:
            message = f'Cannot find tag "{tag}" for {guild.system.value.title()}!'
    elif len(matches):
        message = ''
...
async def ability_command(
    ...
    if ctx.guild_id in vermissian.player_guilds:
        ...
        system_abilities: Dict[str, Ability] = ability_command.abilities[guild.system.value]
        ...
        if lower_ability_to_use in system_abilities:
            ...
            message = f'Cannot find ability "{ability_to_use}" for {guild.system.value.title()}!'
    else:
        message = ''
...
async def delve_draw_command(
    ...
    def pick_card(picked: Set[str]) -> Tuple[str, Dict[str, Union[str, List[str]]]]:
        keep_picking = True

        card_name = None
        card = None
        while keep_picking:
            suit: str = random.choice(list(cards.keys()))

            card_value: Union[str, int] = random.choice(list(cards[suit].keys()))

            card = cards[suit][card_value]

            card_name = f'{card_value} of {suit}s'

            if not allow_duplicates and card_name in picked:
                keep_picking = True
            else:
                keep_picking = False
    ...
    for i in range(3):
        picked_name, picked_card = pick_card(picked_cards)
    ...
    if five_card_draw:
        positive_flavour_name, positive_flavour_card = pick_card(picked_cards)
        picked_cards.add(positive_flavour_name)

        negative_flavour_name, negative_flavour_card = pick_card(picked_cards)
...
async def add_character_command(
    ...
    character = vermissian.add_character(ctx.guild_id, spreadsheet_url=character_sheet_url, user=ctx.user)
...
async def get_skills(ctx: discord.AutocompleteContext) -> List[str]:
    ...
    if game_type == System.SPIRE:
        return [skill.value for skill in SpireSkill]
    elif game_type == System.HEART:
        return [skill.value for skill in HeartSkill]
...
async def get_domains(ctx: discord.AutocompleteContext) -> List[str]:
    ...
    if game_type == System.SPIRE:
        return [domain.value for domain in SpireDomain]
    elif game_type == System.HEART:
        return [domain.value for domain in HeartDomain]
...
async def roll_action_command(
    ...
    if guild.system == System.SPIRE:
        ...
        highest, results, outcome, total, had_skill, had_domain, did_downgrade = vermissian.roll_check(
        ...
        highest, results, outcome, total, had_skill, had_domain, used_difficult_actions_table = vermissian.roll_check(
            guild_id=ctx.guild_id,
            user=ctx.user,
            initial_roll=roll,
            skill=skill_to_use,
        ...
        raise ValueError(f'Unknown game type: {guild.system}')
...
async def get_resistance_tracks(ctx: discord.AutocompleteContext) -> List[str]:
    if ctx.interaction.guild_id not in vermissian.player_guilds:
        ...
        # Fun fact: The below lines just break stuff! So we get janky workarounds instead.
        # await ctx.interaction.respond('You must link to a character tracker via /link before you can do this.')
        # raise ValueError(f'You must link to a character tracker via /link before you can do this.')

    game_type = vermissian.player_guilds[ctx.interaction.guild_id].system

    if game_type == System.SPIRE:
        return SpireGame.RESISTANCES
    elif game_type == System.HEART:
        return HeartGame.RESISTANCES
...
async def roll_fallout_command(
    ...
    resistance_options = guild.RESISTANCES
...
async def link_command(
    ...
    if ctx.guild_id in vermissian.player_guilds:
        vermissian.remove_game(ctx.guild_id)

        await ctx.respond('Unlinked! You can re-link by using /link.')
    else:
        await ctx.respond('No game to unlink.')
...
async def link_command(
    ...
    player_guild = vermissian.player_guilds[ctx.guild_id]

    if len(player_guild.character_sheets):
        for discord_username, character_sheet in player_guild.character_sheets.items():
...
def main():
    ...
    for server_data_dir in glob.glob(os.path.join('servers', '*')):
        ...
        try:
            ...
            vermissian.add_game(guild_id=guild_id, game=game)
