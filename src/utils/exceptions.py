...
class BadCharacterKeeperError(BotError):
...
class NoSpreadsheetGidError(BotError):
...
class UnknownSystemError(BotError):
...
class WrongDifficultyError(BotError):
...
class NotARollError(BotError):
    def __init__(self, msg: str = 'This was not a roll.', * args):
        super().__init__(msg, * args)

class ForbiddenSpreadsheetError(BotError):
...
class TooManyRequestsError(BotError):
