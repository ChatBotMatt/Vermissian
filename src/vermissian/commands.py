...
{bullet(f'Fix the {code("/roll")} tooltip.')}
{bullet('Fix invalid rolls not properly outputting an error message.')}
{bullet('Suggestion response is now ephemeral.')}
'''
...
def get_ability(ability: str, system: Optional[System] = None):
    if not hasattr(get_ability, 'abilities'):
        with open('all_abilities_vermissian.json', 'r', encoding='utf-8') as f:
    ...
    if system is not None: # TODO Find closest match if no matches?
        ...
        if lower_ability_to_use in system_abilities:
            ...
            logger.warning(f'Tried to find unknown ability "{ability_to_use} derived from "{ability}".')
...
def roll_heart_action(game: HeartGame, username: str, skill: str, domain: str, mastery: bool = False, num_helpers: int = 0, difficulty: int = 0):
    ...
    roll = Roll(num_dice=num_dice, dice_size=dice_size, cut=Cut(num=difficulty, threshold=0))
...
def simple_roll(rolls: List[Roll], note: Optional[str] = None):
    ...
    for index, roll in enumerate(rolls):
        ...
        if (roll.num_dice - roll.drop) > 0:
            ...
            formatted_rolls = ResistanceGame.format_roll(results, indices_to_remove, effective_highest)
